...
TEST_CASE("para law: projecting .second degenerates to fold_fix (Nat)") {
    ...
    auto para_algebra =
        [](const NatF<std::pair<Nat, int>> &layer) -> int {
        return std::visit(
            overloaded{
                [](const Zero &) { return 0; },
    ...
    for (int n = 0; n <= 10; ++n) {
        ...
        CHECK(para<int>(para_algebra, nat) == fold_fix<int>(plain_algebra, nat));
    }
}
...
TEST_CASE("para law: projecting .second degenerates to fold_fix (IntList)") {
    ...
    for (const auto &v :
        ...
        CHECK(para<int>(para_algebra, list) == fold_fix<int>(plain_algebra, list));
    }
}
    ...
    auto algebra =
        ...
        return std::visit(
            overloaded{
                ...
                [](const Cons<int, std::pair<IntList, std::vector<std::vector<int>>>>
                    ...
                    std::vector<int> original_tail =
                        smd::fixpoint::list_to_vector((*c.tail).first);
