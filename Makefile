...
# cmake/beman-deps.cmake (S07d, beman::execution/beman::task) is NOT part
# of the top-level includes: it is include()d by its consumer,
# src/smd/fixpoint/CMakeLists.txt, immediately before the
# FetchContent_MakeAvailable() that needs its declares. That keeps the
# Makefile flow and the Beman preset/matrix CI flows (which set only
# infra/cmake/use-fetch-content.cmake as the top-level include) on one
# provisioning path.
ifeq ($(VCPKG),)
...
install: $(_build_path)/CMakeCache.txt compile ## Install the project
	$(CMAKE) --install $(_build_path) --config $(CONFIG) --component fixpoint.fixpoint_Development --verbose
