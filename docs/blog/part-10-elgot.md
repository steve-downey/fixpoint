<div class="abstract" id="org502e202">
<p>
...
`refold` with a `match` wrapped around the step: Left returns the answer directly &#x2014; note that the algebra is never called, not even once, if the *first* seed short-circuits; Right proceeds exactly as `refold` would.
...
The examined-count is not decoration &#x2014; it is the *only* observable evidence of the short-circuit. The product alone proves nothing: multiplying by zero swallows whether the tail was visited. The header makes the same point about testing, and it generalizes: a plausible off-by-one &#x2014; expand one extra layer, then discard it &#x2014; still gets every numeric answer right, so the test suite counts coalgebra invocations instead of trusting outputs. When a scheme's contract is about *work avoided*, test the work, not the answer.
...
No short-circuit here &#x2014; the extra power is context, not control. It is `para`'s move transplanted to hylomorphisms: `para`'s algebra saw the original subtree next to each result; `coelgot`'s sees the original *seed* next to the folded layer, in a refold where no subtree ever exists to show.

One implementation note earns its comment: `coalgebra(seed)` is bound to a local before the `fmap`. A careless transcription that calls `psi` twice &#x2014; once to build the layer, once inside the fold &#x2014; would be invisible to every test with a pure coalgebra and would silently double the cost, and the side effects, of an impure one. The elgot/coelgot pair is small; the discipline in the comments is most of its value.
...
Every one is "same three-line worker, richer type at one position", and each side's riches line up with the other's &#x2014; products and annotations feeding algebras, sums and chunks flowing from coalgebras. The catalog is complete except for the reveal: Part 11 shows the one construction all the algebra-side schemes are instances of, the one all the coalgebra-side schemes are instances of, and the single moving part &#x2014; a distributive law &#x2014; that selects among them.
